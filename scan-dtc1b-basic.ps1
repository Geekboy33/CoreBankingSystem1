...
$blockSize = 10 * 1024 * 1024  # 10MB por bloque
...
try {
    # Limitar a primeros 50 bloques para prueba
    $maxBlocks = [Math]::Min(50, $totalBlocks)
...
Write-Host "Bytes procesados: $([Math]::Round($processedBytes / 1GB, 2)) GB" -ForegroundColor Cyan
...
# Crear resultados
$completeResults = @{
    scanId = "BASIC_SCAN_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
    mode = "BASIC_DTC1B_SCAN"
    progress = @{
        ...
        totalBytes = $fileSize
        averageSpeedMBps = [Math]::Round(($processedBytes / 1MB) / $totalTime.TotalSeconds, 2)
...
Write-Host "=== ESCANEO BÁSICO FINALIZADO ===" -ForegroundColor Green
