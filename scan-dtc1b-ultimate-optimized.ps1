inge# SCRIPT ULTIMATE OPTIMIZADO - SOLUCIONA TODOS LOS PROBLEMAS
param(
...
Write-Host "=== SCRIPT ULTIMATE OPTIMIZADO ===" -ForegroundColor Cyan
...
# Patrones optimizados sin caracteres especiales
$Patterns = @{
    Balance = @(
...
function Start-UltimateScan {
    try {
        ...
        Write-Host "Modo ULTIMATE: Activado" -ForegroundColor Green
        ...
        $finalResults | ConvertTo-Json -Depth 10 | Out-File (Join-Path $OutputDir "final-results-ultimate.json") -Encoding UTF8
        
        Write-Host "`nArchivos guardados:" -ForegroundColor Cyan
        Write-Host "Resultados finales: $OutputDir\final-results-ultimate.json" -ForegroundColor Green
