# Script final para escanear 800 GB de datos DTC1B
param(
    [string]$DTC1BPath = "E:\dtc1b"
)

Write-Host "=== ESCANEO FINAL DTC1B - 800 GB ===" -ForegroundColor Cyan
...
Write-Host "Total EUR: €$($totalEUR.ToString('N2'))" -ForegroundColor Green
Write-Host "Total USD: $$($totalUSD.ToString('N2'))" -ForegroundColor Green
Write-Host "Total GBP: £$($totalGBP.ToString('N2'))" -ForegroundColor Green

Write-Host "`n=== ESTADÍSTICAS DETALLADAS ===" -ForegroundColor Yellow
...
foreach ($balance in $eurBalances | Select-Object -First 10) {
    Write-Host "$($balance.File): €$($balance.Balance.ToString('N2'))" -ForegroundColor Green
}
...
foreach ($transaction in $topTransactions | Select-Object -First 10) {
    $symbol = if ($transaction.Currency -eq "EUR") { "€" } elseif ($transaction.Currency -eq "USD") { "$" } elseif ($transaction.Currency -eq "GBP") { "£" } else { $transaction.Currency }
    Write-Host "$($transaction.File): $symbol$($transaction.Amount.ToString('N2')) ($($transaction.Currency))" -ForegroundColor Green
}

Write-Host "`n=== CUENTAS ÚNICAS ENCONTRADAS ===" -ForegroundColor Yellow
...
$results | ConvertTo-Json -Depth 10 | Out-File "dtc1b-final-scan-results.json" -Encoding UTF8
Write-Host "`nResultados guardados en: dtc1b-final-scan-results.json" -ForegroundColor Cyan
