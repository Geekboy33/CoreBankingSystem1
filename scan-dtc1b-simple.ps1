...
$blockSize = 5 * 1024 * 1024  # 5MB por bloque
...
try {
    # Limitar a primeros 20 bloques para prueba
    $maxBlocks = [Math]::Min(20, $totalBlocks)
...
# Crear resultados
$completeResults = @{
    scanId = "SIMPLE_SCAN_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
    mode = "SIMPLE_DTC1B_SCAN"
    progress = @{
...
Write-Host "=== ESCANEO SIMPLE FINALIZADO ===" -ForegroundColor Green
